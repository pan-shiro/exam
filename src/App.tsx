...
function App() {
  ...
  const handleOperations = () => {
    const { answers: currentOptions, correct_answer, multiple_correct_answers } = questions[currentIndex];
  ...
  return (
    <div className="h-screen bg-slate-300">
      ...
      <main className="px-4 py-2 mt-4 w-full h-1/2 flex justify-center items-center">
        {quizStatus === 'idle' && 
        <StartingScreen selectedCategory={selectedCategory} categories={categories} onSelectOptionChange={(e) => setSelectedCategory(e.target.value)} onStartQuizBtnClick={() => setQuizStatus('begun')} />
        }
        {quizStatus === 'begun' && questions.length > 0 ? <section className="w-1/2 min-h-72 p-2 mx-auto bg-slate-100 rounded-lg shadow-xl box-border">
          ...
          <form className="px-3" onSubmit={handleSubmit}>
            ...
            <ul className="px-4">
              {Object.values(questions[currentIndex].answers).filter((ans) => ans !== null).map((ans, i) => (
                <li className="my-2 hover:bg-indigo-100 rounded-lg" key={ans}>
                  {questions[currentIndex].multiple_correct_answers !== "true" ?
                    <label className="align-middle font-mono w-full block">
                      <input className="mx-4 align-middle" type="radio" name={currentIndex.toString()} value={Object.keys(questions[currentIndex].answers)[i]} onChange={handleOptions} />
                      {ans}
                    </label>
                    :
                    <label className="align-middle font-mono w-full block">

                      <input className="mx-4 align-middle" type="checkbox" name={currentIndex.toString()} value={Object.keys(questions[currentIndex].answers)[i]} onChange={handleOptions} />
                      {ans}
                    </label>
                  }
                </li>
            ...
            <div className="flex justify-end">
              <button className="rounded-lg bg-indigo-500 px-4 py-2 min-w-24 text-indigo-100 hover:bg-indigo-600 disabled:bg-indigo-300 disabled:cursor-not-allowed" type="submit" onClick={() => console.log(multipleOptions.length)} disabled={!selectedOption}>{currentIndex < questions.length - 1 ? "Next" : "Submit answers"} </button>
            </div>
          </form>
